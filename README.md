...
| 模块 | 页面 | 说明 |
| --- | --- | --- |
| 智能剪辑 | `pages/index` | 上传比赛视频 → 选择精彩片段 → AI 模拟生成集锦；顶部问候语按时段显示用户名（早上好/下午好/晚上好，姓名取自本地资料），右上角头像进入个人中心（微信头像/昵称快捷区、编辑资料、主题切换、AI 助手、创建比赛） |
...
- 头像选择两条路：微信官方头像选择器（`button open-type="chooseAvatar"`）与从相册/相册+拍摄（`wx.chooseMedia`）。
...
- 微信已回收 `wx.getUserInfo` / `wx.getUserProfile`，**无法静默获取**微信头像与微信名；本项目改用官方**头像昵称填写能力**，且必须由用户本人点击一次：
  - 头像：`<button open-type="chooseAvatar" bind:chooseavatar="...">`，回调 `e.detail.avatarUrl` 为**临时路径**，需 `wx.getFileSystemManager().saveFile()` 落盘后再持久化。
  - 昵称：`<input type="nickname">`，用户在输入时选择「使用微信昵称」，通过 `bindnickname` / `bindinput` 拿到 `e.detail.value`。
- 入口在首页右上角头像弹层的「微信头像 / 昵称」快捷区：点「微信头像」走微信头像选择器、点「相册」走本地图片、昵称框选完点「保存」；写入本地 `profile` 后首页问候语与弹层资料卡立即刷新。
- 基础库要求 ≥ 2.21.2（本项目 `project.config.json` 的 `libVersion` 为 3.15.0，满足）。
- 头像编辑页「个人头像」亦接入微信头像选择器，相册上传入口保留。
...
**版本信息**：v4.4.0（接入微信官方头像昵称填写能力：首页弹层快捷区 + 编辑资料页头像选择器）
**最后更新**：2026-09-21
